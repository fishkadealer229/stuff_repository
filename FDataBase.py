...
class FDataBase:
    ...
    def addUserinfo(self, name, surname, age, gender, locality, work_place, username, password):
        try:
            self.__cur.execute(
                f"UPDATE users SET name='{name}', surname='{surname}', age='{age}', gender='{gender}', locality='{locality}', work_place='{work_place}', username='{username}', password='{password}'")
            self.__db.commit()
        except sqlite3.Error as e:
            print("Ошибка изменения данных в БД " + str(e))
            return False
